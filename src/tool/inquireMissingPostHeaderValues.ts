...
export default async function inquireMissingPostHeaderValues(given: Partial<PostHeader>): Promise<PostHeader> {
    const questions: (UnnamedDistinctQuestion & {name: string})[] = [{
        ...
        filter: category => category.trim().length === 0 ? null : category.trim()
    }, {
        name: 'tags',
        type: 'input',
        message: 'Tags (separated with comma)?',
        filter: (tags: string) => {
            const parsed = tags.split(',').map(i => i.trim())
                .filter(i => i.length !== 0)
                .reduce((prev, cur) => {
    ...
    const remaining = await inquirer.prompt(questions.filter(i => !givenKeys.includes(i.name)))
