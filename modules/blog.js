...
export default class Blog {
    ...
    async getArticleNames() {
        ...
        return names.filter((i) => i.endsWith('.md'))
    ...
    async getArticleList() {      
        ...
        return posts.sort((a, b) => (a.metadata.date && b.metadata.date) ? Date.parse(b.metadata.date) - Date.parse(a.metadata.date) : 0);
    }
    ...
    async getCategories(withCount = false) {
        ...
        if (withCount)
            ...
            return articles.reduce((prev, cur) => {
                if (!prev.includes(cur.metadata.category))
